...
int post_process(uint8_t *input0, uint8_t *input1, uint8_t *input2, int model_in_h, int model_in_w,
                 ...
                 detect_result_group_t *group, const char* labelFilePath)
{

    LOGD("post_process");
    // LOGD("label file path: %s", labelFilePath);
    static int init = -1;
    if (init == -1)
    {
        int ret = 0;
        ret = loadLabelName(labelFilePath, labels);
        if (ret < 0)
        {
            LOGD("load label name failed");
            return -1;
        }
    ...
    LOGD("model_in_h: %d, model_in_w: %d", model_in_h, model_in_w);
    ...
    LOGD("validCount0 %d",validCount0);
    ...
    LOGD("validCount1 %d",validCount1);
    // stride 32
    int stride2 = 32;
    ...
    LOGD("validCount2 %d",validCount2);
